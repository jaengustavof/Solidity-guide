...
contract example_mapping_iterator{
    ...
    function getRegisteredStudentsWallet() public view returns (address[] memory)
    {
        return idAlumnos;
    }

    function getAllStudents() public view returns (Student[] memory){
        Student[] memory studentList = new Student[](idAlumnos.length);
        for(uint i = 0; i<idAlumnos.length;i++) {
            studentList[i] = alumnos[idAlumnos[i]];
        }
        return studentList;
    }

    function getAllStudentNames() public view returns (string[] memory){
        string[] memory studentNames = new string[](idAlumnos.length);
        for(uint i = 0; i<idAlumnos.length;i++) {
            studentNames[i] = alumnos[idAlumnos[i]].name;
        }
        return studentNames;
    }
